...
class Keyboard:
    ...
    def start_kb(self):
        ...
        builder.button(text='Мы в ТГ', url='google.com')
        builder.button(text='Мы в ВК', url='google.com')
    ...
    def how_to_kb(self, ind):
        ...
        if ind == 0:
            ...
            builder.button(text='←', callback_data='how_to_5')
            builder.button(text='Меню', callback_data='start_all')
            builder.adjust(1, 1)
        else:
    ...
    def faq(self, nind=-1):
        builder = InlineKeyboardBuilder()
        texts = ['О Poizon', 'О нас', 'О доставке', 'Почему нам можно доверять', 'Наш ассортимент']
        for ind, txt in enumerate(texts):
            if ind == nind:
                builder.button(text='👟 ' + txt, callback_data=f'faq_{ind}')
            else:
                builder.button(text=txt, callback_data=f'faq_{ind}')
        builder.button(text='🔙', callback_data='start_all')
        builder.adjust(1)
