...
async def start_all(call: CallbackQuery, state: FSMContext):
    if db.open_connect():
        ...
        await bot2.send_message(chat_id=admin_id, text='Новый запрос в очереди')
        await call.message.answer(
            f"""Спасибо за ваше обращение. Менеджер скоро вернется к вам.
Если вам не ответили в течение 2 часов, дадим вам скидку на доставку в 15%""")
    else:
        db.add_talk(call.from_user.id, txt=f'Пользователь хочет, чтобы вы с ним связались\n@{call.from_user.username}')
        txt2 = f'Вы общаетесь с пользователем @{call.from_user.username}'
        message = await bot2.send_message(chat_id=admin_id, text=txt2, reply_markup=kb.stop_talk(call.from_user.id))
        await state.set_state('talking')
        await bot2.pin_chat_message(chat_id=admin_id,message_id=message.message_id)
        await call.message.answer(
            f"""Спасибо за ваше обращение. Менеджер скоро вернется к вам.
        Если вам не ответили в течение 2 часов, дадим вам скидку на доставку в 15%""")
    await call.answer()
...
Наш менеджер скоро свяжется с вами для уточнения всех подробностей. 

Если вы не получили ответа в течение 2х часов, то для вас будет скидка на доставку 15%""",reply_markup=kb.main_kb())
    else:
        await call.message.answer(
    f"""Спасибо за ваш заказ!
Наш менеджер скоро свяжется с вами для уточнения всех подробностей. 

Если вы не получили ответа в течение 2х часов, то для вас будет скидка на доставку 15%""",
            reply_markup=kb.main_kb())
        db.add_talk(call.from_user.id, txt=txt)
...
async def start_all(call: CallbackQuery, state: FSMContext):
    await call.message.delete()
    await call.message.answer(f"Пришлите стоимость товара")
...
async def last_caaszxatcher(message: Message, state: FSMContext):
    curs = db.get_curs()
    try:
        await message.answer(f"Стоимость вашего товара - {int(float(curs) * int(message.text) + 1000)}")
        time.sleep(2)
...
async def start_all(call: CallbackQuery):
    await call.message.edit_caption(caption='''• Мы доставим ваш заказ в срок от 10 до 20 дней.\n\n• Быстрее вы не найдете нигде.\n\n• Доставляем с помощью СДЭК, Почты России.\n\n• Весь процесс перемещения товара мы предоставляем по вашему запросу.\n\n• Сумма доставки может быть разная, но варьируется от 600 до 1500 рублей.\n\n• Ее нужно уточнять индивидуально, поскольку она зависит от веса вашего заказа.\n\n• Подробнее можете прочитать в нашей статье!
\n\nhttps://vk.com/@poizonshippin-dostavka''', reply_markup=kb.back())
...
async def start_all(call: CallbackQuery):
    if '_' in call.data:
        ...
        await call.message.edit_caption(caption=faq_text[ind], reply_markup=kb.faq(ind))
    else:
        await call.message.edit_caption(
            caption='В разделе FAQ вы можете посмотреть все частые ответы на неменее частые вопросы)',
            reply_markup=kb.faq())
